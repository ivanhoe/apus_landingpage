...
#   - Ex: docker.io/hexpm/elixir:1.19.5-erlang-28.1-debian-trixie-20260223-slim
#
...
RUN apt-get update \
  && apt-get install -y --no-install-recommends build-essential git nodejs npm librsvg2-bin \
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/apus_landing ./
